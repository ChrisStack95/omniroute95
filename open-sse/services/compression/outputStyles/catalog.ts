...
export interface OutputStyle {
  ...
  boundaries?: string;
  /** Optional localized `boundaries`, keyed by language code; falls back to `boundaries` then SHARED_BOUNDARIES. */
 ...
 * `ponytail` and `less-code` shape code, not prose, so they carry this clause
 * INSTEAD of the prose-oriented SHARED_BOUNDARIES.
 */
...
export const OUTPUT_STYLE_CATALOG: Record<string, OutputStyle> = {
  ...
  "less-code": {
    ...
    // YAGNI prompt drops guards in upstream's benchmark) instead of the
    // prose-oriented SHARED_BOUNDARIES.
