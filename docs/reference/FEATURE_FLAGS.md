...
| `STREAM_RECOVERY_TOOLCALL_ORDER_FIX`        | boolean | `false` |         | Resume trailing text from the same upstream connection after a transient mid-stream cut once a tool call has completed, using order-aware in-flight detection. Off by default: order-blind detection plus sticky latch.                                                                                                                                                                                                                                              |
