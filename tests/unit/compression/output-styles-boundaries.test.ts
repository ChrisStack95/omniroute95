 ...
 *   - applyOutputStyles() emits the per-style boundary instead of
 *     SHARED_BOUNDARIES for such styles (single style and multi-style)
...
function injected(
  ...
  return String(r.body.messages?.[0]?.content);
}

test("per-style boundary is emitted instead of SHARED_BOUNDARIES (single style)", () => {
  const text = injected(
  ...
  assert.ok(
    !text.includes(SHARED_BOUNDARIES),
    "SHARED_BOUNDARIES must not leak into a carve-out style"
  );
...
test("less-code emits the carve-out too (its levels no longer double-append boundaries)", () => {
  const text = injected(
    { messages: [{ role: "user", content: "Refactor this module." }] },
    sel(["less-code", "ultra"])
  );
...
test("multi-style: carve-out style + non-boundary style emits both clauses in catalog order", () => {
  // catalog order: terse-prose, less-code → SHARED_BOUNDARIES then the carve-out.
  const text = injected(
    { messages: [{ role: "user", content: "Refactor this module." }] },
  ...
  assert.ok(sharedAt < carveAt, "clauses follow catalog order");
});

test("multi-style: two carve-out styles deduplicate to a single clause", () => {
  const text = injected(
    { messages: [{ role: "user", content: "Refactor this module." }] },
